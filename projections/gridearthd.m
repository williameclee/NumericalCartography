...
%	origin - which longitude (in degrees) to centre the projection on
...
%
% Notes
%	The version with radian inputs is GRIDEARTH.
%
% See also
%	GRIDEARTH
%
...
function varargout = gridearthd(varargin)
    %% Initialisation
    % Demos
    ...
    % Parse inputs
    [lond, latd, lonOrigind, g, addAnchors, outputFormat, typeflag] = ...
        parseinputs(varargin{:});

    %% Passing on to the main function
    % Convert to radians
    lon = deg2rad(lond);
    lat = deg2rad(latd);
    lonOrigin = deg2rad(lonOrigind);
    [X, Y] = gridearth(lon, lat, lonOrigin, ...
        "Geometry", g, "Anchors", addAnchors, "OutputFormat", outputFormat);
